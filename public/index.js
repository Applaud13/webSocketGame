...
socket.on(SOCKET.STAGECHANGE, (data) => {
  score.stage = data.stage;
  score.stageClearScore = data.clearScore;
  score.scorePerTime = data.scorePerTime;
  score.stageId = data.id;
  score.stageChange = true;
});

socket.on(SOCKET.LOADASSETS, (assets) => {
  itemUnlock = assets.item_unlocks.data;
  itemController.itemUnlock = assets.item_unlocks.data;
  for (let i = 0; i < ITEM_CONFIG.length; i++) {
    ITEM_CONFIG[i].score = assets.items.data[i].score;
  }
});
...
// 선인장
const CACTI_CONFIG = [
  { width: 48 / 1.5, height: 100 / 1.5, image: "images/obstacle1.png" },
...
function gameLoop(currentTime) {
  ...
  if (!gameover && !waitingToStart) {
    ...
    itemController.update(gameSpeed, deltaTime, score.stageId);
    // 달리기
  ...
  if (collideWithItem && collideWithItem.itemId) {
    const grabedIetm = ITEM_CONFIG.find(
      (item) => item.id === collideWithItem.itemId
    );
    score.grabItem(grabedIetm);
    sendEvent(5, {
      itemId: grabedIetm.id,
      itemScore: grabedIetm.score,
      timeStamp: Date.now(),
    });
  }
